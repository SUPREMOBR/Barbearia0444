...
if ($total_registro > 0) {
	...
	for ($i = 0; $i < $total_registro; $i++) {
		...
		echo '<li>';
		echo '<a href="#" class="item-link item-content" onclick="editarServicos(' . $id . ', \'' . $descricao . '\', \'' . $valorF . '\', \'' . $nome_pessoa . '\', \'' . $data_vencimentoF . '\', \'' . $data_pagamentoF . '\', \'' . $tumb_arquivo . '\', \'' . $nome_usuario_lancou . '\', \'' . $nome_usuario_pagamento . '\', \'' . $foto . '\', \'' . $ext . '\', \'' . $oc . '\', \'' . $pagamento . '\')">';
		echo ' <div class="item-inner">';
		echo ' <div class="item-title" style="font-size:11px">';
		echo ' <div class="item-header " style="font-size:9px"><i class="mdi mdi-square" style="color:' . $classe_alerta . '"></i> ' . $descricao . ' - (' . $data_lancamentoF . ')</div>R$' . $valorF . ' (' . $nome_pessoa . ')';
		echo '<div class="item-footer" style="font-size:9px">Data pagamento: ' . $data_vencimentoF . ' (' . $pagamento . ')</div>';
		echo '</div>';
