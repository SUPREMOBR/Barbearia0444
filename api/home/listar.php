...
$total_pagar_mes = 0;
$query = $pdo->query("SELECT * FROM pagar where data_vencimento >= '$data_inicio_mes' and data_vencimento <= '$data_final_mes' and pago != 'Sim'");
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
if (@count($resultado) > 0) {
    for ($i = 0; $i < @count($resultado); $i++) {
        foreach ($resultado[$i] as $key => $value) {
        }
        $total_pagar_mes += $resultado[$i]['valor'];
    }
}
$total_pagar_mesF = number_format($total_pagar_mes, 2, ',', '.');

$total_receber_mes = 0;
$query = $pdo->query("SELECT * FROM receber where data_vencimento >= '$data_inicio_mes' and data_vencimento <= '$data_final_mes' and pago != 'Sim'");
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
if (@count($resultado) > 0) {
    for ($i = 0; $i < @count($resultado); $i++) {
        foreach ($resultado[$i] as $key => $value) {
        }
        $total_receber_mes += $resultado[$i]['valor'];
    }
}
...
$total_compra_mes = 0;
$query = $pdo->query("SELECT * FROM pagar where data_vencimento >= '$data_inicio_mes' and data_vencimento <= '$data_final_mes' and pago != 'Sim' and tipo = 'Compra'");
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
if (@count($resultado) > 0) {
    for ($i = 0; $i < @count($resultado); $i++) {
        foreach ($resultado[$i] as $key => $value) {
        }
        $total_compra_mes += $resultado[$i]['valor'];
    }
}
$total_compra_mesF = number_format($total_compra_mes, 2, ',', '.');

$total_comissao_mes = 0;
$query = $pdo->query("SELECT * FROM pagar where data_vencimento >= '$data_inicio_mes' and data_vencimento <= '$data_final_mes' and pago != 'Sim' and tipo = 'Comissão'");
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
if (@count($resultado) > 0) {
    for ($i = 0; $i < @count($resultado); $i++) {
        foreach ($resultado[$i] as $key => $value) {
        }
        $total_comissao_mes += $resultado[$i]['valor'];
    }
}
...
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
$total_agendamentos_usuario = @count($resultado);
...
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
$total_agendamentos_usuario_conc = @count($resultado);
...
if (@count($resultado) > 0) {
    for ($i = 0; $i < @count($resultado); $i++) {
        ...
        $total_comissao_pendente += $resultado[$i]['valor'];
    }
}
...
$query = $pdo->query("SELECT * FROM pagar where pago != 'Sim' and tipo = 'Comissão' and funcionario = '$id_usuario' and data_lancamento = curDate()");
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
if (@count($resultado) > 0) {
    for ($i = 0; $i < @count($resultado); $i++) {
        foreach ($resultado[$i] as $key => $value) {
        }
        $total_comissao_pendente_hoje += $resultado[$i]['valor'];
    }
}
...
$query = $pdo->query("SELECT * FROM receber where tipo = 'Serviço' and funcionario = '$id_usuario' and data_lancamento = curDate()");
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
$total_servicos_executado = @count($resultado);
if (@count($resultado) > 0) {
    for ($i = 0; $i < @count($resultado); $i++) {
        foreach ($resultado[$i] as $key => $value) {
        }
        $total_servicos_hoje += $resultado[$i]['valor'];
    }
}
