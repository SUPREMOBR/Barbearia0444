...
if($filtro == ''){
	$acao_relatorio = 'Entradas / Ganhos';
}elseif($filtro == 'Venda'){
		$acao_relatorio = ' Vendas ';
}elseif($filtro == 'Serviço'){
		$acao_relatorio = ' Serviços ';
}else{
		$acao_relatorio = 'Recebimentos';
}
...
<body>	
	...
	<div class="mx-2" >
		...
		<?php 
		$total_entradas = 0;
		$query = $pdo->query("SELECT * FROM receber where data_pagamento >= '$dataInicial' and data_pagamento <= '$dataFinal' and tipo LIKE '$filtro' and pago = 'Sim' ORDER BY data_pagamento asc");
