...
if ($pagamento == '') {
	...
	$acao_relatorio = 'Pagamento: ' . $pagamento;
}
...
<body>
	...
	<div class="titulo_cab titulo_img"><u>Relatório de Serviços <?php echo $titulo_servico ?> <?php echo $acao_relatorio ?> </u></div>
	<div class="data_img"><?php echo mb_strtoupper($data_hoje) ?></div>

	<img class="imagem" src="<?php echo $url_sistema ?>/sistema/img/logo_relatorio.jpg" width="150px">
	...
	<div class="mx-2">
		...
		<?php
		$total_entradas = 0;
		$query = $pdo->query("SELECT * FROM receber where data_pagamento >= '$dataInicial' and data_pagamento <= '$dataFinal' and tipo = 'Serviço' and pago = 'Sim' and servico LIKE '$servico' and pgto LIKE '$pagamento'  ORDER BY data_pagamento asc");
		$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
		$total_registro = @count($resultado);
		if ($total_registro > 0) {
		?>
