...
if($filtro == ''){
	$acao_relatorio = 'Saídas / Despesas';
}elseif($filtro == 'Compra'){
		$acao_relatorio = ' Compras ';
}elseif($filtro == 'Comissão'){
		$acao_relatorio = ' Comissões ';
}else{
		$acao_relatorio = 'Despesas';
}
...
<body>	
	...
	<div class="titulo_cab titulo_img"><u>Relatório de <?php echo $acao_relatorio ?> </u></div>	
	...
	<div class="mx-2" >
		...
		<?php 
		$total_entradas = 0;
		$query = $pdo->query("SELECT * FROM pagar where data_pagamento >= '$dataInicial' and data_pagamento <= '$dataFinal' and tipo LIKE '$filtro' and pago = 'Sim' ORDER BY data_pagamento asc");
  ...
  <tbody>
	...
	$data_pagamento = $resultado[$i]['data_pagamengto'];
	...
	$data_lancamentoFormatada = implode('/', array_reverse(explode('-', $data_lancamento)));
	$data_pagamentoFormatada = implode('/', array_reverse(explode('-', $data_pagamento)));
	$data_vencimentoFormatada = implode('/', array_reverse(explode('-', $data_vencimento)));
...
<td class="esc"><?php echo $data_pagamentoFormatada ?></td>
...
exit();
	...
	<div class="col-md-12 p-2">
		...
		<span class="text-danger"> <small><small><small><small>TOTAL R$</small> : <?php echo @$total_entradasFormatada ?></small></small></small>  </span>
