...
<body>
	...
	and pago = 'Sim' ORDER BY data_pagamento asc");
				...
				//totalizar os vendas 
				$query = $pdo->query("SELECT * FROM receber where data_pagamento >= '$dataInicial' and data_pagamento <= '$dataFinal' 
	and tipo = 'Venda' and pago = 'Sim' ORDER BY data_pagamento asc");
				$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
				$total_registro = @count($resultado);
				for ($i = 0; $i < $total_registro; $i++) {
					foreach ($resultado[$i] as $key => $value) {
				...
				$query = $pdo->query("SELECT * FROM receber where data_pagamento >= '$dataInicial' and data_pagamento <= '$dataFinal' 
	and tipo = 'Conta' and pago = 'Sim' ORDER BY data_pagamento asc");
				$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
				$total_registro = @count($resultado);
				for ($i = 0; $i < $total_registro; $i++) {
					foreach ($resultado[$i] as $key => $value) {
				...
				//totalizar comandas
				$query = $pdo->query("SELECT * FROM receber where data_pagamento >= '$dataInicial' and data_pagamento <= '$dataFinal' and tipo = 'Comanda' 
	and pago = 'Sim' ORDER BY data_pagamento asc");
				$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
				$total_registro = @count($resultado);
				for ($i = 0; $i < $total_registro; $i++) {
				...
				$query = $pdo->query("SELECT * FROM pagar where data_pagamento >= '$dataInicial' and data_pagamento <= '$dataFinal' and tipo = 'Conta' 
	and pago = 'Sim' ORDER BY data_pagamento asc");
				$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
				$total_registro = @count($resultado);
				for ($i = 0; $i < $total_registro; $i++) {
				...
				$query = $pdo->query("SELECT * FROM pagar where data_pagamento >= '$dataInicial' and data_pagamento <= '$dataFinal' and tipo = 'Compra' 
	and pago = 'Sim' ORDER BY data_pagamento asc");
				$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
				$total_registro = @count($resultado);
				for ($i = 0; $i < $total_registro; $i++) {
				...
				//totalizar contas despesas
				$query = $pdo->query("SELECT * FROM pagar where data_pagamento >= '$dataInicial' and data_pagamento <= '$dataFinal' and tipo = 'Comissão' 
	and pago = 'Sim' ORDER BY data_pagamento asc");
				$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
				$total_registro = @count($resultado);
				for ($i = 0; $i < $total_registro; $i++) {
