...
$tabela = 'marketing'; // Define o nome da tabela no banco de dados

// Realiza a consulta para selecionar clientes que possuem telefone válido e não nulo
$query = $pdo->query("SELECT * FROM clientes where telefone != '' and telefone is not null");
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
$total_clientes = @count($resultado); // Conta o número de clientes com telefone válido

// Consulta para obter os registros de marketing ordenados pelo ID em ordem decrescente
$query = $pdo->query("SELECT * FROM $tabela ORDER BY id desc");
$resultado = $query->fetchAll(PDO::FETCH_ASSOC);
$total_registro = @count($resultado); // Conta o número total de registros na tabela de marketing

// Verifica se há registros de marketing para exibir
if ($total_registro > 0) {
	// Início da estrutura HTML de uma tabela para exibir os dados
	echo <<<HTML
	<small>
	...
	<tbody>	
HTML;
	// Itera sobre cada registro encontrado para exibir suas informações na tabela
	for ($i = 0; $i < $total_registro; $i++) {
		foreach ($resultado[$i] as $key => $value) {
		}
		// Captura todos os campos para o registro atual
		$id = $resultado[$i]['id'];
		$titulo = $resultado[$i]['titulo'];
		$mensagem = $resultado[$i]['mensagem'];
		$item1 = $resultado[$i]['item1'];
		$item2 = $resultado[$i]['item2'];
		$item3 = $resultado[$i]['item3'];
		$item4 = $resultado[$i]['item4'];
		$item5 = $resultado[$i]['item5'];
		$item6 = $resultado[$i]['item6'];
		$item7 = $resultado[$i]['item7'];
		$item8 = $resultado[$i]['item8'];

		$item9 = $resultado[$i]['item9'];
		$item10 = $resultado[$i]['item10'];
		$item11 = $resultado[$i]['item11'];
		$item12 = $resultado[$i]['item12'];
		$item13 = $resultado[$i]['item13'];
		$item14 = $resultado[$i]['item14'];
		$item15 = $resultado[$i]['item15'];
		$item16 = $resultado[$i]['item16'];
		$item17 = $resultado[$i]['item17'];
		$item18 = $resultado[$i]['item18'];
		$item19 = $resultado[$i]['item19'];
		$item20 = $resultado[$i]['item20'];

		$arquivo = $resultado[$i]['arquivo']; // Caminho do arquivo associado ao registro
		$audio = $resultado[$i]['audio'];     // Caminho do áudio associado ao registro
		$data_envio = $resultado[$i]['data_envio']; // Data do último envio
		$data = $resultado[$i]['data'];             // Data de criação do registro
		$envios = $resultado[$i]['envios'];         // Total de envios realizados
		$forma_envio = $resultado[$i]['forma_envio']; // Forma de envio utilizada

		$hash = $resultado[$i]['hash'];
		$hash2 = $resultado[$i]['hash2'];
		$hash3 = $resultado[$i]['hash3'];

		// Formata as datas no formato dia/mês/ano
		$data_envioF = implode('/', array_reverse(@explode('-', $data_envio)));
		$dataF = implode('/', array_reverse(@explode('-', $data)));

		// Define a exibição do áudio
		$ocultar_audio = 'ocultar';
		if ($audio != "") {
			$ocultar_audio = ''; // Mostra o campo de áudio se existir um arquivo
		}
		// Define o tipo de envio padrão para "Todos" se estiver vazio
		if ($forma_envio == "") {
			$forma_envio = "Todos";
		}
