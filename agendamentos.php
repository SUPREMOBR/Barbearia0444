						...
						<select onchange="mudarServico()" class="form-control sel2" id="servico" name="servico" style="width:100%;" required>
							...
							if ($total_registro > 0) {
								for ($i = 0; $i < $total_registro; $i++) {
									...
									echo '<option value="' . $resultado[$i]['id'] . '">' . $resultado[$i]['nome'] . ' - R$ ' . $valorFormatado . '</option>';
								}
							}
							?>
		...
		$.ajax({
			...
			success: function(mensagem) {
				...
				if (mensagem.trim() == "Agendado com Sucesso") {
					...
					var horaFormat = hora.split(':');
					var horaHora = horaFormat[0];
					var horaMinuto = horaFormat[1];
					var horaFormatada = horaHora + ':' + horaMinuto;
					...
					if (msg_agendamento == 'Sim') {
						...
						a.target = '_blank';
						a.href = 'http://api.whatsapp.com/send?1=pt_BR&phone=<?= $telefone_whatsapp ?>&text= _Novo Agendamento_ %0A Funcionário: *' + nome_funcionario + '* %0A Serviço: *' + nome_serv + '* %0A Data: *' + dataFormatada + '* %0A Hora: *' + horaFormatada + '* %0A Cliente: *' + nome + '*  %0A %0A ' + obs;
						a.click();
						return;
...
<script type="text/javascript">
	function listarFuncionario() {
		var funcionario = $("#funcionario").val(); // var func
